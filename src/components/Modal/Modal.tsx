...
const Modal: FC<ModalProps> = ({
	...
	const contentStyles = `mt-2 relative bg-white rounded-xl shadow-lg transition-all duration-300 overflow-auto ${customSize === 'small' ? 'pb-4 max-h-[35vh] w-full sm:w-2/3 md:w-1/2 lg:w-1/3' : 'max-h-[90vh] w-full max-w-[90vh]'} `;
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
			...
			<div className={contentStyles}>
				<div className={titleStyles}>
					...
					<button aria-label="close" onClick={() => closeModal(name)}>
						<GrClose
							className={`h-5 w-5 ${title ? 'text-white' : 'text-base'}`}
						/>
					</button>
				</div>
				<div>{children}</div>
